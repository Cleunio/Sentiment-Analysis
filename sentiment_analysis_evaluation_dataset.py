# -*- coding: utf-8 -*-
"""Sentiment_Analysis_Evaluation_Dataset.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/18drPWmhM_eUuYNRAfszTce66aOPsWQxP
"""
...
text_positive = "I like the colours of yours eyes"
text_positive = preprocessing(text_positive)
text_positive

prevision = model_load(text_positive)
prevision

prevision.cats

text_negative = "I hate the colours of yours eyes"
text_negative = preprocessing(text_negative)
prevision2 = model_load(text_negative)
prevision2.cats

prev_ = []
for text_ in dataset['text']:
  #print(text_)
  prev = model_load(text_)
  prev_.append(prev.cats)

prev_final = []
for prev in prev_:
  if prev['HAPPY'] > prev['FEAR']
    prev_final.append('HAPPY')
  else:
    prev_final.append('FEAR')

prev_final= np.array(prev_final)

prev_final

answer_real = dataset['emotion'].values

accuracy_score(answer_real, prev_final)

cm = confusion_matrix(answer_real, prev_final)
cm

dataset_test = pd.read_csv('dataset_test.csv', encoding = 'utf-8')

dataset_test.head()

dataset_test['text'] = dataset_test['text'].apply(preprocessing)

dataset_test.head()

prev_text = []
for text_ in dataset_test['text']:
  #print(text_)
  prev_t = model_load(text_)
  prev_text.append(prev_t.cats)
...
answer_real = dataset_test['emotion'].values
